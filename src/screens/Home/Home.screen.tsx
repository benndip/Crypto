...
const Home = () => {
  ...
  const { data: trendingSearchData, error: trendingSearchError, isLoading: loadingTrendingSearch } = useGetTrendingSearchQuery({})

  console.log("Trending markets", trendingSearchData);
  
  const renderTrendingCoin = ({ item }:{item:CoinType}) => {
    return (
      <TrendingCryptoItem coin={item} />
    )
  }
      ...
      <View style={styles.sendButtonRecieve}>
        <View style={styles.iconAndText}>
          <View style={styles.iconContanier}>
            <Feather name="arrow-up-right" size={24} color={PRIMARY_WHITE_COLOR} />
          </View>
          <Text style={styles.sendText}>Send</Text>
        </View>
        <View style={styles.iconAndText}>
          <View style={[styles.iconContanier, { backgroundColor: '#315BF0', borderWidth: 0 }]}>
        ...
        <View style={styles.iconAndText}>
          <View style={styles.iconContanier}>
            <Feather name="arrow-down-left" size={24} color={PRIMARY_WHITE_COLOR} />
          </View>
          <Text style={styles.receiveText}>Receive</Text>
        </View>
      </View>
      <View style={{ paddingLeft: DEVICE_WIDTH * 0.05, marginTop: DEVICE_HEIGHT * 0.05  }}>
